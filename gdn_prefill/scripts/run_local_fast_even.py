...
This variant rewrites each prefill workload's `cu_seqlens` so the original
`total_seq_len` is split as evenly as possible across the original `num_seqs`.

Usage:
  cd scripts/
  uv run python run_local_fast_even.py                          # download dataset from HuggingFace
  uv run python run_local_fast_even.py --local /path/to/dataset  # use local dataset
  uv run python run_local_fast_even.py --run_baseline gdn_prefill  # run FlashInfer baseline
"""
...
def build_even_cu_seqlens(total_seq_len: int, num_seqs: int, device: str) -> torch.Tensor:
    if num_seqs <= 0:
        raise ValueError(f"Expected num_seqs > 0, got {num_seqs}")

    base_len, remainder = divmod(total_seq_len, num_seqs)
    lengths = torch.full((num_seqs,), base_len, dtype=torch.int64, device=device)
    if remainder:
        lengths[:remainder] += 1
...
def main(args: argparse.Namespace):
    ...
    for workload in workloads:
        ...
        even_cu_seqlens = build_even_cu_seqlens(total_seq_len, num_seqs, device)
        inputs[cu_seqlens_idx] = even_cu_seqlens
        ...
        even_seq_len_min = total_seq_len // num_seqs
        even_seq_len_max = even_seq_len_min + int(total_seq_len % num_seqs > 0)

        sample = dict(
            uuid=workload.uuid,
            **workload.axes,
            even_seq_len_min=even_seq_len_min,
            even_seq_len_max=even_seq_len_max,
            cu_seqlens_mode="even",
        )
        correct = False
        ...
        print(workload.uuid, workload.axes, f"even_seq_len=[{even_seq_len_min}, {even_seq_len_max}]", f"{correct=}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fast single-process benchmark with even cu_seqlens")
    parser.add_argument(
        "--run_baseline",
