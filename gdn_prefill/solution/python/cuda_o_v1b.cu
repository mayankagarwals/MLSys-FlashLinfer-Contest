...
constexpr uint32_t OFFSET_H = OFFSET_K + K_SMEM_BYTES;
...
__global__ __block_size__((NUM_THREADS, 1, 1)) void o_v1b_kernel_cutlass(
  ...
  if (warp_id == TMA_WARP) {
    ...
    for (uint32_t global_chunk_id = blockIdx.y, chunk_iter = 0U;
      ...
      const uint32_t v_stage_smem = k_smem + qk_phase * K_SMEM_SIZE;
...
#pragma unroll
      for (uint32_t head_offset = 0; head_offset < HEADS_PER_QK_HEAD;
        ...
        if (elect_sync()) {
          ...
          tma_load_4d(v_stage_smem, &v_tmap, 0, chunk_start_i32, v_tile, head_id,
                      v_tma_barrier);
    ...
    for (uint32_t global_chunk_id = blockIdx.y, chunk_iter = 0U;
      ...
      const uint32_t v_stage_smem = k_stage_smem;
