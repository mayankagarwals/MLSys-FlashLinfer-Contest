...
// ═══════════════════════════════════════════════════════════════════
// Prep chunk metadata kernel — runs on 1 thread, computes chunk_indices
// and chunk_offsets from cu_seqlens. Replaces the Triton compute_chunks_kernel.
// ═══════════════════════════════════════════════════════════════════
...
prep_meta_kernel(
  ...
  // Each thread handles one sequence (up to 57 sequences)
  if (tid < num_seqs) {
...
// Combined: prep_meta + kkt_v1b in 2 kernel launches from 1 C++ call
void kkt_v1b_with_meta(
  TensorView K,
  TensorView A_log,
  TensorView a,
  TensorView dt_bias,
  ...
  TensorView cu_seqlens,
  TensorView chunk_indices,
  TensorView chunk_offsets  // [N+1] — full array, last element is total_chunks
) {
  const int T = K.size(0);
  const int Hg = K.size(1);
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(kkt_v1b_with_meta, kkt_v1b_with_meta);
