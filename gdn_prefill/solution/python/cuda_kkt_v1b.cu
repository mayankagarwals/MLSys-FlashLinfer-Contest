// same as kkt_v1, but uses [H, total_T (padded)] layout for beta and g_cu
...
void kkt_v1_kernel_cutlass(
  ...
  const nv_bfloat16 *b_ptr,                    // [total_T, H]
        float       *g_cu_ptr,                 // [H, total_T (padded)]
        float       *beta_ptr,                 // [H, total_T (padded)]
  ...
  if (warp_id == NUM_WARPS - 1) {
    ...
    for (int global_chunk_id = bid; global_chunk_id < total_num_chunks; global_chunk_id += gridDim.x) {
      ...
      if (off_t < eos)
        beta_ptr[head_id * stride_g + off_t] = beta;
      ...
      // store to gmem for future use
      if (off_t < eos)
        g_cu_ptr[head_id * stride_g + off_t] = g;
...
void kkt_v1(
  ...
  const int stride_g = g_cu.stride(0);
  ...
  kernel<<<grid, TB_SIZE, smem_size>>>(
    ...
    cu_seqlens_ptr, chunk_indices_ptr, stride_g, total_num_chunks);
}
