 ...
 *    - OOutputKernel (tcgen05, BV=64): parallel output computation
 *      3 MMAs per chunk, embarrassingly parallel across all chunks
 *      Uses s_qh smem buffer to avoid global read-modify-write
 *
 * 3. TCGEN05 FOR ALL KERNELS
 *    - OKernel now uses tcgen05 TMEM (same as HKernel)
 *    - All 4 MMAs (q@k^T, q@h^T, attn@vnew in O-kernel) use tcgen05 tensor cores
 *    - cp.async loads for q, k, h; manual scatter for vnew
 *    - q@k^T computed inline in O-kernel (eliminates FusedPrep Phase 4)
 *
 ...
 * 5. OOutputKernel:     Output computation (tcgen05 TMEM, 4 MMAs/chunk parallel)
 *                       Includes inline q@k^T with gating (moved from FusedPrepKernel)
 *
 * tcgen05 tile layout:
