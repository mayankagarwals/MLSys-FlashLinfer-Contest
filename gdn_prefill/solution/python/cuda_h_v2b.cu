...
template <int BV, int NUM_STAGES, bool DO_PROFILE>
__global__
__block_size__((TB_SIZE, 1, 1))
void h_v2b_kernel_cutlass(
  ...
  const uint32_t tma_mbar     = v_scale_smem + v_scale_size;
  const uint32_t wh_in_mbar   = tma_mbar + NUM_STAGES * 8;
  const uint32_t wh_done_mbar = wh_in_mbar + NUM_STAGES * 8;
  const uint32_t vk_in_mbar   = wh_done_mbar + NUM_STAGES * 8;
  const uint32_t vk_done_mbar = vk_in_mbar + NUM_STAGES * 8;

  const uint32_t h0_mbar_addr = vk_done_mbar + NUM_STAGES * 8;
  ...
  if (warp_id == 0) {
    ...
    if (elect_sync()) {
      for (int i = 0; i < NUM_STAGES; i++) {
        mbarrier_init(tma_mbar + i * 8, 1);                // 1 TMA
        mbarrier_init(wh_in_mbar + i * 8, WARP_SIZE * 8);  // h from H warps and v from V warps
        mbarrier_init(wh_done_mbar + i * 8, 1);            // 1 MMA
        mbarrier_init(vk_in_mbar + i * 8, WARP_SIZE * 8);  // scaled_h from H warps and scaled_v from V warps
        mbarrier_init(vk_done_mbar + i * 8, 1);            // 1 MMA
      }
  ...
  if (warp_id == NUM_WARPS - 1) {
    ...
    if (elect_sync()) {
      ...
      tma_load_4d(H0_f32_smem, &H0_tmap, 0, v_id * BV, 0, seq_id * H + head_id, h0_mbar_addr, EVICT_FIRST);
      mbarrier_arrive_expect_tx(h0_mbar_addr, H_fp32_size);

      for (int chunk_id = 0; chunk_id < num_chunks; chunk_id++) {
        // compute addresses
        const int off_t = bos + chunk_id * BT;
        const int off_t_uw = (chunk_offset + chunk_id) * BT;
        const uint32_t W_smem = smem + stage_id * STAGE_SIZE;
        const uint32_t V_smem = W_smem + W_size;
        const uint32_t K_smem = V_smem + V_size;
        const uint32_t mbar_addr = tma_mbar + stage_id * 8;
  
        // wait MMA warp to release the buffer
        mbarrier_wait(vk_done_mbar + stage_id * 8, parity);
        profiler.stamp(WAIT_MMA);

        // issue TMA and arrive
        // natural shape: [T, H, dim]
        // permute shape: [H, dim/64, T, 64]
        tma_load_4d(W_smem, &W_tmap, 0, off_t_uw, 0, head_id, mbar_addr, EVICT_FIRST);
        tma_load_4d(V_smem, &V_tmap, 0, off_t_uw, v_id * BV / 64, head_id, mbar_addr, EVICT_FIRST);
    ...
    auto process = [&](int chunk_id) {
      ...
      if (chunk_id == 0) {
        ...
        if (warp_id_ == 0) {
          mbarrier_wait(vk_done_mbar + vk_stage_id * 8, vk_parity);
        ...
        for (int i = 0; i < K_dim / 32; i++) {
          ...
          if (chunk_id == 0) {
            ...
            for (int j = 0; j < 32 / 4; j++) {
              ...
              lds_f32x4(h_f32 + j * 4, H0_f32_smem + i * BV * 128 + tid_ * 128 + col * 16);
            }
          }
          else {
          ...
          // for O kernel
          // H smem layout: [K_dim/64, BV, 64]
          // each j iter:   [       1, 32,  8]
          for (int j = 0; j < 32 / 8; j++) {
            const int col = ((i % 2) * 4 + j) ^ (tid_ % 8);
            sts_b32x4(H_smem + (i / 2) * BV * 128 + tid_ * 128 + col * 16, tmp + j * 4);
          }
        }
      }
      if constexpr (BV == 64) {
        ...
        for (int i = 0; i < K_dim / 32; i++) {
          ...
          for (int j = 0; j < 8; j++)
            tmp[j] = fp32x2_to_bf16x2(h_f32[j * 2], h_f32[j * 2 + 1]);
          tcgen05_st<SHAPE::_16x128b, 4>(warp_id_ * 32, h_tmem_base + i * 16, tmp);
          ...
          for (int j = 0; j < 32 / 16; j++) {
            ...
            const int col = ((i % 2) * 4 + j * 2 + (lane_id / 16)) ^ (lane_id % 8);
            stmatrix<4>(H_smem + (i / 2) * BV * 128 + row * 128 + col * 16, tmp + j * 4);
          }
        }
      }
      tcgen05_wait_st();
      tcgen05_fence_before_thread_sync();
      mbarrier_arrive(wh_in_mbar + stage_id * 8);
      ...
      tcgen05_wait_st();
      tcgen05_fence_before_thread_sync();
      mbarrier_arrive(vk_in_mbar + stage_id * 8);
      ...
      if (warp_id_ == 3 && elect_sync()) {
        ...
        tma_store_4d(&H_tmap, H_smem, 0, v_id * BV, 0, (chunk_offset + chunk_id) * H + head_id);
    ...
    if constexpr (BV == 128) {
      ...
      for (int i = 0; i < K_dim / 32; i++) {
        ...
        for (int j = 0; j < 32 / 4; j++) {
          const int col = j ^ (tid_ % 8);
          sts_b32x4(H0_f32_smem + i * BV * 128 + tid_ * 128 + col * 16, h_f32 + j * 4);
        }
      }
    }
    if constexpr (BV == 64) {
      // smem layout: [K_dim/32, BV, 32] with swizzling
      // per warp:    [K_dim/32, 16, 32]
      // per i iter:  [       1, 16, 32]
      // per j iter:  [       1, 16,  8]
      for (int i = 0; i < K_dim / 32; i++) {
        uint32_t h_f32[16];
        tcgen05_ld<SHAPE::_16x128b, 8>(h_f32, warp_id_ * 32, vk_tmem + i * 32);

        for (int j = 0; j < 32 / 8; j++) {
          const int row = warp_id_ * 16 + (lane_id % 16);
          const int col = (j * 2 + (lane_id / 16)) ^ (lane_id % 8);
          stmatrix<4>(H0_f32_smem + i * BV * 128 + row * 128 + col * 16, h_f32 + j * 4);
        }
      }
    }
    bar_sync<1>(128);
    ...
    if (warp_id_ == 0 && elect_sync()) {
      // natural shape:  [N, H, V_dim, K_dim]
      // permuted shape: [N*H, K_dim/64, V_dim, 64]
      tma_store_4d(&HT_tmap, H0_f32_smem, 0, v_id * BV, 0, seq_id * H + head_id);
      cp_async_bulk_commit_group();
    }
    if (warp_id_ == 1) {
    ...
    for (int chunk_id = 0; chunk_id < num_chunks; chunk_id++) {
      ...
      if constexpr (BV == 128) {
        // V tmem layotu: [BV, BT]
        // V smem layout: [BV/64, BT, 64]
        // each warp:     [    1, BT, 32]
        // each ldmatrix: [    1,  8, 32]
      ...
      if constexpr (BV == 64) {
        ...
        // V smem layout: [BV/64, BT, 64]
        // each warp:     [    1, BT, 16]
        // each ldmatrix: [    1, 16, 16]
        for (int i = 0; i < BT / 16; i++) {
          uint32_t v_bf16[4];
          const uint32_t s_row = i * 16 + (lane_id / 16) * 8 + (lane_id % 8);
          const uint32_t s_col = (warp_id * 2 + ((lane_id / 8) % 2)) ^ (lane_id % 8);
          ldmatrix_trans<4>(v_bf16, V_smem + s_row * 128 + s_col * 16);

          float v_fp32[8];
          for (int k = 0; k < 4; k++)
            bf16x2_to_fp32x2(v_fp32 + k * 2, v_bf16[k]);

          tcgen05_st<SHAPE::_16x256b, 2>(warp_id * 32, wh_tmem + i * 16, v_fp32);
        }
      }
      tcgen05_wait_st();
      tcgen05_fence_before_thread_sync();
      ...
      if (warp_id == 2) {
        // wait for wh MMA
        mbarrier_wait(wh_done_mbar + stage_id * 8, parity);
      }
      else if (warp_id == 3) {
      ...
      if constexpr (BV == 128) {
        ...
        for (int i = 0; i < BT / 8; i++) {
          ...
          for (int k = 0; k < 4; k++) {
            ...
            v_tmp[k] = fp32x2_to_bf16x2(tmp[k * 2], tmp[k * 2 + 1]);
      ...
      if constexpr (BV == 64) {
        ...
        for (int i = 0; i < BT / 16; i++) {
          float tmp[8];
          tcgen05_ld<SHAPE::_16x256b, 2>(tmp, warp_id * 32, wh_tmem + i * 16);

          uint32_t v_tmp[4];
          float2 v_scale[2];
          v_scale[0] = reinterpret_cast<float2 *>(v_scale_smem_ptr + (i * 16 + 0 + (lane_id % 4) * 2))[0];  // first 8
          v_scale[1] = reinterpret_cast<float2 *>(v_scale_smem_ptr + (i * 16 + 8 + (lane_id % 4) * 2))[0];  // next 8
          ...
          for (int k = 0; k < 4; k++) {
            ...
            // scale v_new for vk MMA
            reinterpret_cast<uint32_t *>(tmp)[k] = fp32x2_to_bf16x2(tmp[k * 2 + 0] * v_scale[k / 2].x,
                                                                    tmp[k * 2 + 1] * v_scale[k / 2].y);
          }
          ...
          // V smem layout: [BV/64, BT, 64]
          // each warp:     [    1, BT, 16]
          // each stmatrix: [    1, 16, 16]
          const uint32_t s_row = i * 16 + (lane_id / 16) * 8 + (lane_id % 8);
          const uint32_t s_col = (warp_id * 2 + ((lane_id / 8) % 2)) ^ (lane_id % 8);
          stmatrix_trans<4>(V_new_smem + s_row * 128 + s_col * 16, v_tmp);

          // store scaled v_new for vk MMA
          tcgen05_st<SHAPE::_16x128b, 2>(warp_id * 32, v_tmem_base + i * 8, tmp);
        }
      }
      tcgen05_wait_st();
      tcgen05_fence_before_thread_sync();
      ...
      if (warp_id == 3 && elect_sync()) {
        ...
        tma_store_4d(&V_new_tmap, V_new_smem, 0, (chunk_offset + chunk_id) * BT, v_id * BV / 64, head_id);
        cp_async_bulk_commit_group();
      }
...
static
CUtensorMap encode_tma(void *ptr, uint64_t T, uint64_t H, uint64_t dim, uint32_t block_dim) {
  CUtensorMap tmap;
  ...
  uint32_t boxDim[rank] = {64, BT, block_dim / 64, 1};
...
static
CUtensorMap encode_h_tma(void *ptr, uint64_t N, uint32_t BV, CUtensorMapDataType dtype) {
  CUtensorMap tmap;
  ...
  uint32_t boxDim[rank] = {num_elems, BV, K_dim / num_elems, 1};
  uint32_t elementStrides[rank] = {1, 1, 1, 1};
...
void h_v2b(
  ...
  auto K_tmap     = encode_tma(K.data_ptr(), T, Hg, K_dim, K_dim);
  auto V_tmap     = encode_tma(V.data_ptr(), V.size(0), H, V_dim, BV);
  auto W_tmap     = encode_tma(W.data_ptr(), W.size(0), H, K_dim, K_dim);
  auto V_new_tmap = encode_tma(V_new.data_ptr(), V_new.size(0), H, V_dim, BV);  // padded layout
  auto H0_tmap    = encode_h_tma(h0.data_ptr(), N, BV, CU_TENSOR_MAP_DATA_TYPE_FLOAT32);
  auto HT_tmap    = encode_h_tma(ht.data_ptr(), N, BV, CU_TENSOR_MAP_DATA_TYPE_FLOAT32);
  auto H_tmap     = encode_h_tma(h.data_ptr(), h.size(0), BV, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16);
  ...
  if (profiler.has_value()) {
    ...
    auto kernel = h_v2b_kernel_cutlass<BV, NUM_STAGES, false>;
    cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
    kernel<<<grid, TB_SIZE, smem_size>>>(
      K_tmap, V_tmap, W_tmap, H0_tmap, HT_tmap, H_tmap, V_new_tmap,
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(h_v2b, h_v2b);
