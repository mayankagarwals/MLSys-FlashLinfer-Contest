...
def fused_ho_kernel(
    ...
    for chunk_id in range(num_chunks):
        ...
        wh = tl.dot(w, tl.trans(h_bf16))  # bf16 MMA [BT, BV]
        ...
        qh = tl.dot(q, tl.trans(h_bf16))  # bf16 MMA [BT, BV]
        exp_g = tl.exp(g_cu)
        ...
        # o_intra uses tf32 (fp32 inputs → tf32 MMA) — higher precision, closer to reference
        o_intra = tl.dot(A.to(v_new.dtype), v_new)  # fp32 inputs → tf32 MMA
