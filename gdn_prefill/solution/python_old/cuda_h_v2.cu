  ...
  if (warp_id == NUM_WARPS - 1) {
    ...
    if (elect_sync()) {
      ...
      for (int chunk_id = 0; chunk_id < num_chunks; chunk_id++) {
        ...
        tma_load_4d(V_smem, &V_tmap, 0, off_t, v_id * BV / 64, head_id, mbar_addr, EVICT_FIRST);
...
void h_v2(
  ...
  auto V_tmap     = encode_tma(V.data_ptr(), T, H, V_dim, BV);
  auto W_tmap     = encode_tma(W.data_ptr(), T, H, K_dim, K_dim);
  ...
  if (profiler.has_value()) {
    ...
    auto kernel = h_v2_kernel_cutlass<BV, NUM_STAGES, true>;
    ...
    auto kernel = h_v2_kernel_cutlass<BV, NUM_STAGES, false>;
    cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
    kernel<<<grid, TB_SIZE, smem_size>>>(
      K_tmap, V_tmap, W_tmap, H0_tmap, HT_tmap, H_tmap, V_new_tmap,
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(h_v2, h_v2);
