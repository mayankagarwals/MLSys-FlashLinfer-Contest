...
def _tiny_recurrent_kernel(
    ...
    """Token-recurrent GDN path for workloads dominated by launch overhead."""
    seq_id = tl.program_id(axis=0)
    head = tl.program_id(axis=1)
    v_block = tl.program_id(axis=2)

    bos = tl.load(cu_seqlens + seq_id).to(tl.int32)
    eos = tl.load(cu_seqlens + seq_id + 1).to(tl.int32)
    seq_len = eos - bos

    v_offsets = v_block * BV + tl.arange(0, BV)
    k_offsets = tl.arange(0, HEAD_DIM)
    state_offsets = (
        ((seq_id * NUM_HEADS + head) * HEAD_DIM + v_offsets[:, None]) * HEAD_DIM
        + k_offsets[None, :]
    )
    if HAS_STATE:
        state_tile = tl.load(state + state_offsets).to(tl.float32)
    else:
        state_tile = tl.zeros((BV, HEAD_DIM), dtype=tl.float32)

    qk_head = head // (NUM_HEADS // NUM_QK_HEADS)
    neg_exp_A = -tl.exp(tl.load(A_log + head).to(tl.float32))
    dt = tl.load(dt_bias + head).to(tl.float32)

    for token_offset in range(seq_len):
        token = bos + token_offset
        token_head_offset = token * NUM_HEADS + head
        qk_base = (token * NUM_QK_HEADS + qk_head) * HEAD_DIM

        q_vec = tl.load(q + qk_base + k_offsets).to(tl.float32)
        k_vec = tl.load(k + qk_base + k_offsets).to(tl.float32)
        v_values = tl.load(
            v + token_head_offset * HEAD_DIM + v_offsets
        ).to(tl.float32)

        a_value = tl.load(a + token_head_offset).to(tl.float32) + dt
        softplus = tl.maximum(a_value, 0.0) + tl.log(
            1.0 + tl.exp(-tl.abs(a_value))
        )
        decay = tl.exp(neg_exp_A * softplus)
        beta_value = tl.load(b + token_head_offset).to(tl.float32)
        beta = 1.0 / (1.0 + tl.exp(-beta_value))

        decayed_state = decay * state_tile
        predicted_v = tl.sum(decayed_state * k_vec[None, :], axis=1)
        delta = beta * (v_values - predicted_v)
        state_tile = decayed_state + delta[:, None] * k_vec[None, :]

        out_values = scale * tl.sum(state_tile * q_vec[None, :], axis=1)
        tl.store(
            output + token_head_offset * HEAD_DIM + v_offsets,
            out_values.to(tl.bfloat16),
        )

    tl.store(new_state + state_offsets, state_tile)
...
def run(q, k, v, state, A_log, a, dt_bias, b, cu_seqlens, scale):
    ...
    # The chunked path pays four launches and computes a padded 64x64 inverse.
    # A single token-recurrent launch is substantially cheaper for tiny inputs.
    if num_seqs <= 2 and total_seq_len <= num_seqs * 30:
        ...
        state_arg = state if state is not None else new_state
        recurrent_bv = 4
        _tiny_recurrent_kernel[
            (num_seqs, num_v_heads, head_size // recurrent_bv)
        ](
            q,
            k,
            v,
    ...
    total_chunks_ptr = chunk_offsets[num_seqs:]
