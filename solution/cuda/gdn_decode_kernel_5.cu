...
static_assert(kHeadSize == 128, "kernel_5 expects head size 128");
static_assert(kNumThreads == 128, "kernel_5 expects 128 threads per block");
static_assert(kHeadSize % kWarpSize == 0,
              "head size must be divisible by warp size");
static_assert(kRowsPerBlock == 4, "kernel_5 expects four rows per block");
static_assert(kHeadSize % kRowsPerBlock == 0,
              "head size must be divisible by rows per block");
static_assert(kElemsPerLane == 4, "kernel_5 expects four elements per lane");
...
__global__ void GdnDecodeKernel5(
...
void LaunchGdnDecodeKernel5(const __nv_bfloat16 *__restrict__ q_ptr,
  ...
  GdnDecodeKernel5<<<grid, kNumThreads, 0, stream>>>(
      q_ptr, k_ptr, v_ptr, state_ptr, A_log_ptr, a_ptr, dt_bias_ptr, b_ptr,
      scale_f, output_ptr, new_state_ptr);
}

void RunGdnDecodeKernel5(TensorView q, TensorView k, TensorView v,
  ...
  LaunchGdnDecodeKernel5(q_ptr, k_ptr, v_ptr, state_ptr, A_log_ptr, a_ptr,
  ...
  if (err != cudaSuccess) {
    TVM_FFI_THROW(RuntimeError)
        << "GdnDecodeKernel5 launch failed: " << cudaGetErrorString(err);
  }
}

} // namespace

TVM_FFI_DLL_EXPORT_TYPED_FUNC(gdn_decode_v5, RunGdnDecodeKernel5);
