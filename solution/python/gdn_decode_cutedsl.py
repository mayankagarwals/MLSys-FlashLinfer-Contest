...
def gdn_decode_kernel_small_batch_pretranspose(
    ...
    for v_tiles in range(start_v_tiles, end_v_tiles):
        for row in cutlass.range_constexpr(0, SMALL_TILE_V, NUM_THREADS // WARP_SIZE):
            ...
            # Load h from global state tile using contiguous K access.
            gSrc_tile = cute.local_tile(
                gSrc, (1, vec_size, 1), (row + row_offset, lane_id, v_tiles)
            )
            cute.autovec_copy(gSrc_tile, r_h)
...
def gated_delta_rule_decode_pretranspose(
    ...
    _, _, HV, V = v.shape
    assert B == 1, f"This kernel variant only supports B=1, got B={B}"

    # Validate K and V constraints
    assert K >= 128, f"K must be at least 128, got K={K}"
    assert V >= 128, f"V must be at least 128, got V={V}"
    assert V % SMALL_TILE_V == 0, (
        f"V must be divisible by {SMALL_TILE_V} to prevent out-of-bounds access, got V={V}"
    )
