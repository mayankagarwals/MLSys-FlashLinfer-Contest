...
def chunk_scaled_dot_kkt_fwd_kernel(
    ...
    k_desc = tl.make_tensor_descriptor(
        k_ptr + k_head_id * K_dim,
        [T, K_dim],
        [Hg * K_dim, 1],
        [BT, K_dim],
    )

    for chunk_id in range(tl.cdiv(seqlen, BT)):
        k = k_desc.load([bos + chunk_id * BT, 0])
...
def run(
    ...
    # instead of launching N x Hg threadblocks, we can launch
    # N x Hg x M threadblocks, where M is a small number, and
    # we can "stride" through the sequence. this ensures we
    # launch sufficiently many threadblocks but not too excessive.
    #
    ...
    chunk_scaled_dot_kkt_fwd_kernel[(Hg, N)](
        k,
        A_log,
        a,
        dt_bias,
