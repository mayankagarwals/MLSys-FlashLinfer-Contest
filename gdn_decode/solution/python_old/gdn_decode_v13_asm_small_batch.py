...
#
# q/k: llvm.inline_asm matches gdn_decode_kernel_7.cu LoadBf16x4GlobalNc (ld.global.nc +
# L1::evict_first + cvt to f32).
# State writeback: llvm.inline_asm matches StoreF32x4RelaxedNoAllocate (st.relaxed.cta.global
# L1::no_allocate.v4.f32). State load: llvm.inline_asm ld.global.v4.f32 (matches kernel_7
# reinterpret_cast float4 load per lane; vec_size must be 4).
...
def gdn_decode_kernel_small_batch_gmem(
    ...
    ptr_q = cute.recast_ptr(q_tile.iterator, dtype=cutlass.BFloat16)
    ptr_k = cute.recast_ptr(k_tile.iterator, dtype=cutlass.BFloat16)
    ptr_h = cute.recast_ptr(g_h.iterator, dtype=cutlass.Float32)
    ptr_q_llvm = ptr_q.to_llvm_ptr()
    ptr_k_llvm = ptr_k.to_llvm_ptr()
    ptr_h_llvm = ptr_h.to_llvm_ptr()

    f32_ty = Float32.mlir_type
    asm_f32x4_ty = llvm.StructType.get_literal([f32_ty, f32_ty, f32_ty, f32_ty])
    _ld_qk_bf16x4_nc = (
        "{\n\t"
        ".reg .b16 h0, h1, h2, h3;\n\t"
    ...
    for i in cutlass.range_constexpr(vec_size, unroll=True):
        r_h[i] = cutlass.Float32(llvm.extractvalue(f32_ty, h_f32x4, [i]))
    ...
    sum_hk = 0.0
    for i in cutlass.range_constexpr(vec_size, unroll=True):
        qfi = llvm.extractvalue(f32_ty, q_f32x4, [i])
        kfi = llvm.extractvalue(f32_ty, k_f32x4, [i])
        r_q[i] = cutlass.Float32(qfi)
        r_k[i] = cutlass.Float32(kfi)
        r_h[i] = r_h[i] * r_g
        sum_hk += r_h[i] * r_k[i]
