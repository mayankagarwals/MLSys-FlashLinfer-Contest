...
__global__ void GdnDecodeSmemPipeline1(
...
void RunGdnDecodeSmemPipeline1(TensorView q, TensorView k, TensorView v,
  ...
  GdnDecodeSmemPipeline1<<<grid, kNumThreads, smem_bytes, stream>>>(
  ...
  if (err != cudaSuccess) {
    TVM_FFI_THROW(RuntimeError)
        << "GdnDecodeSmemPipeline1 launch failed: " << cudaGetErrorString(err);
  }
}

} // namespace

TVM_FFI_DLL_EXPORT_TYPED_FUNC(gdn_decode_smem_pipeline_1, RunGdnDecodeSmemPipeline1);
