...
// ============================================================================
// Pipelined kernel: grid = B * HV * kNumCTAs.
// Each CTA handles kVTilesPerCTA=4 v-tiles with cp.async double-buffering.
// Optimised for large batch (B >= 48).
// ============================================================================
...
__global__ void GdnDecodePipelinedV9(
...
#pragma unroll
    for (int iter = 0; iter < kItersPerTile; ++iter) {
      ...
      // Issue v load EARLY to hide latency behind h compute + warp reduce.
      float v_scalar = 0.0f;
      if (lane == 0) {
        v_scalar = __bfloat162float(v[v_offset_i]);
      }
...
void RunGdnDecodeKernel9(TensorView q, TensorView k, TensorView v,
  ...
  GdnDecodePipelinedV9<<<grid, kNumThreads, smem_bytes, stream>>>(
  ...
  if (err != cudaSuccess) {
    TVM_FFI_THROW(RuntimeError)
        << "GdnDecodeKernel9 launch failed: " << cudaGetErrorString(err);
  }
}

} // namespace

TVM_FFI_DLL_EXPORT_TYPED_FUNC(gdn_decode_v9, RunGdnDecodeKernel9);
