...
- Source JSON: `moe/moe_fp8_block_scale_ds_routing_topk8_ng8_kg4_e32_h7168_i2048.json`
- Operation Type: `moe`
- Description: FP8 block scale MoE operation. Routing and two grouped-GEMM included.
- Tags: `status:verified`, `model:deepseek-v3`, `model:deepseek-r1`, `quantization:float8_e4m3fn`

## Axes

| Axis | Type | Value | Description |
| --- | --- | --- | --- |
| `seq_len` | `var` | `-` | Sequence length (number of tokens) |
| `num_experts` | `const` | `256` | Total number of experts. |
| `num_local_experts` | `const` | `32` | Number of local experts with EP size 8. |
| `hidden_size` | `const` | `7168` | Hidden dimension size. |
| `intermediate_size` | `const` | `2048` | MoE intermediate layer size. |
| `gemm1_out_size` | `const` | `4096` | Output size of the first GEMM (W13). Should be 2 * intermediate_size |
| `num_hidden_blocks` | `const` | `56` | Number of quantized blocks along the hidden_size dimension (block_size=128). |
| `num_intermediate_blocks` | `const` | `16` | Number of quantized blocks along the intermediate_size dimension (block_size=128). |
| `num_gemm1_out_blocks` | `const` | `32` | Number of quantized blocks along the gemm1_out_size dimension (block_size=128). |

## Inputs

| Name | Shape | Dtype | Optional | Description |
| --- | --- | --- | --- | --- |
| `routing_logits` | `["seq_len", "num_experts"]` | `float32` | `no` | Tensor of routing logits for expert selection |
| `routing_bias` | `["num_experts"]` | `bfloat16` | `no` | Bias tensor for routing. Pass all zeros for no bias. |
| `hidden_states` | `["seq_len", "hidden_size"]` | `float8_e4m3fn` | `no` | Input hidden states tensor (FP8 quantized) |
| `hidden_states_scale` | `["num_hidden_blocks", "seq_len"]` | `float32` | `no` | Block-wise scaling factors for hidden states. |
| `gemm1_weights` | `["num_local_experts", "gemm1_out_size", "hidden_size"]` | `float8_e4m3fn` | `no` | First GEMM weights for all local experts (gate and up projections). |
| `gemm1_weights_scale` | `["num_local_experts", "num_gemm1_out_blocks", "num_hidden_blocks"]` | `float32` | `no` | Block-wise scaling factors for first GEMM weights. |
| `gemm2_weights` | `["num_local_experts", "hidden_size", "intermediate_size"]` | `float8_e4m3fn` | `no` | Second GEMM weights for all local experts (down projection). |
| `gemm2_weights_scale` | `["num_local_experts", "num_hidden_blocks", "num_intermediate_blocks"]` | `float32` | `no` | Block-wise scaling factors for second GEMM weights. |
| `local_expert_offset` | `scalar` | `int32` | `no` | Offset of local experts in global expert space. |
| `routed_scaling_factor` | `scalar` | `float32` | `no` | Scaling factor for routing weights. |

## Outputs

| Name | Shape | Dtype | Optional | Description |
| --- | --- | --- | --- | --- |
| `output` | `["seq_len", "hidden_size"]` | `bfloat16` | `no` | Final MoE output tensor |
